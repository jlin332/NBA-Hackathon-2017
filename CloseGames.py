...
def closeGames(filename):
    ...
    lastRow = {'id': '(lastRowDiff,startTime'}
    # time the "run" starts
    with open(filename, 'rU') as f:
        ...
        for row in reader:
            ...
            player10 = row[39]
            time = row[14]
            # we don't need to record end time because it's always 0
            lastRow[game_id] = (int(homePts) - int(visitorPts))
            if (int(homePts) - int(visitorPts)
                    == 10 or int(homePts) - int(visitorPts) == -10) and int(period) == 4 and lastRow[game_id][0] <= 2:
                if not game_id in games.keys():
                    games[game_id] = [player1, player2, player3,
                                    player4, player5, player6, player7, player8, player9, player10, time]
    return [games, lastRow]
